...
class SumoApiService {
  ...
  async syncBashoToDatabase(bashoData) {
    try {
      ...
      if (error) {
        // If RLS error and no service key, skip basho sync but don't fail the whole operation
        if (error.message.includes('row-level security') && !supabaseAdmin) {
          console.warn('Skipping basho sync due to RLS policy (no service key available)');
          return bashoData.id; // Return successfully to continue with rikishi sync
        }
        throw error;
      }
  ...
  // Fetch all matches for current tournament
  async fetchAllMatches(bashoId) {
    try {
      const response = await fetch(`${this.apiBase}/basho/${bashoId}/matches`);
      const data = await response.json();
      ...
      console.error(`Error fetching matches for basho ${bashoId}:`, error);
      throw error;
    }
  }

  // Sync matches to database
  async syncMatchesToDatabase(bashoId) {
    try {
      console.log(`Starting match sync for basho ${bashoId}...`);
      const client = supabaseAdmin || supabase;
      
      // Try to fetch matches for the tournament
      let matches = [];
      try {
        matches = await this.fetchAllMatches(bashoId);
      } catch (apiError) {
        console.warn('Could not fetch matches from API, trying individual rikishi approach...');
        
        // Fallback: get matches for each rikishi individually
        const { data: rikishi } = await client
          .from('rikishi')
          .select('id, sumo_api_id, name')
          .not('sumo_api_id', 'is', null);
        
        if (!rikishi || rikishi.length === 0) {
          throw new Error('No rikishi with API IDs found for match sync');
        }
        
        for (const wrestler of rikishi) {
          try {
            const rikishiMatches = await this.fetchRikishiMatches(wrestler.sumo_api_id, bashoId);
            matches.push(...rikishiMatches);
          } catch (error) {
            console.warn(`Failed to fetch matches for ${wrestler.name}:`, error.message);
          }
        }
      }
      
      if (matches.length === 0) {
        console.warn('No matches found for this tournament');
        return { synced: 0, errors: 0, skipped: 1 };
      }
      
      let synced = 0, errors = 0, updated = 0;
      
      for (const match of matches) {
        try {
          // Map API rikishi to our database IDs
          const eastRikishi = await this.findRikishiByApiId(match.eastRikishi?.id);
          const westRikishi = await this.findRikishiByApiId(match.westRikishi?.id);  
          const winner = match.winner ? await this.findRikishiByApiId(match.winner.id) : null;
          
          if (!eastRikishi || !westRikishi) {
            console.warn(`Skipping match - could not find rikishi in database`);
            continue;
          }
          
          const matchData = {
            basho_id: bashoId,
            division: match.division || 'Makuuchi',
            day: match.day || 1,
            match_no: match.matchNumber || synced + 1,
            east_rikishi_id: eastRikishi.id,
            west_rikishi_id: westRikishi.id,
            winner_rikishi_id: winner?.id || null,
            kimarite: match.kimarite || null,
            match_date: match.date ? new Date(match.date).toISOString().split('T')[0] : null
          };
          
          // Upsert match (update if exists, insert if new)
          const { error } = await client
            .from('matches')
            .upsert(matchData, {
              onConflict: 'basho_id,division,day,match_no'
            });
          
          if (error) {
            if (error.message.includes('row-level security') && !supabaseAdmin) {
              console.warn('Match sync blocked by RLS policy - service key needed');
              errors++;
              continue;
            }
            throw error;
          ...
          errors++;
          console.error('Error syncing individual match:', error.message);
        }
      }
      
      console.log(`Match sync completed: ${synced} synced, ${errors} errors`);
  ...
  // Helper: Find rikishi by API ID
  async findRikishiByApiId(apiId) {
    ...
    try {
      ...
      const { data, error } = await client
        .from('rikishi')
        .select('id, name')
        .eq('sumo_api_id', apiId)
        .single();
      
      if (error && error.code !== 'PGRST116') { // PGRST116 = no rows found
        console.warn(`Error finding rikishi with API ID ${apiId}:`, error.message);
      }
      
      return data;
    } catch (error) {
      console.warn(`Failed to find rikishi with API ID ${apiId}:`, error.message);
      return null;
    }
  }
  ...
  async performFullSync() {
    try {
      ...
      console.log(`Service role available: ${!!supabaseAdmin}`);
      ...
      for (const rikishi of apiRikishi) {
        try {
          ...
          // If RLS error and no service key, provide helpful message
          if (error.message.includes('row-level security') && !supabaseAdmin) {
            console.warn('RLS policy blocked operation - service role key needed for full sync');
          }
        }
      }
      ...
      // Try to sync current basho
      let bashoResult = null;
      try {
        const bashoData = await this.fetchCurrentBasho();
        const bashoId = await this.syncBashoToDatabase(bashoData);
        bashoResult = { id: bashoId, location: bashoData.location, isActive: bashoData.isActive };
      } catch (bashoError) {
        console.warn('Basho sync failed, continuing with rikishi data only:', bashoError.message);
      }
      ...
      return { 
        ...
        updated, 
        errors, 
        bashoId: bashoResult?.id || null,
        bashoLocation: bashoResult?.location || 'Unknown',
        totalRikishi: apiRikishi.length,
        serviceRoleAvailable: !!supabaseAdmin,
        warnings: !supabaseAdmin ? ['Limited sync due to missing service role key'] : []
      };
    } catch (error) {
  ...
  // Sync matches for current tournament
  async performMatchSync() {
    try {
      console.log('Starting match sync...');
      console.log(`Service role available: ${!!supabaseAdmin}`);
      
      // Get current basho info
      const bashoData = await this.fetchCurrentBasho();
      
      if (!bashoData.isActive) {
        console.warn('No active tournament found - match sync may not have current data');
      }
      
      // Sync matches for current tournament
      const matchResult = await this.syncMatchesToDatabase(bashoData.id);
      
      return {
        bashoId: bashoData.id,
        bashoLocation: bashoData.location,
        isActive: bashoData.isActive,
        matches: matchResult,
        serviceRoleAvailable: !!supabaseAdmin,
        warnings: !supabaseAdmin ? ['Limited match sync due to missing service role key'] : []
      };
    } catch (error) {
      console.error('Match sync failed:', error);
      throw error;
  ...
  // Combined sync: rikishi + basho + matches
  async performFullSyncWithMatches() {
    try {
      console.log('Starting full sync with matches...');
      
      // First do the standard rikishi and basho sync
      const standardSync = await this.performFullSync();
      
      // Then add match sync if we have a basho
      let matchResult = null;
      if (standardSync.bashoId) {
        try {
          console.log('Adding match sync to full sync...');
          matchResult = await this.syncMatchesToDatabase(standardSync.bashoId);
        } catch (matchError) {
          console.warn('Match sync failed during full sync:', matchError.message);
          matchResult = { synced: 0, errors: 1, total: 0, error: matchError.message };
        }
      }
