...
class SumoApiService {
  ...
  async syncRikishi(apiRikishi) {
    try {
      ...
      const { data: existingRikishi } = await supabase
        .from('rikishi')
        .select('id')
      ...
      if (existingRikishi) {
        ...
        // Create new rikishi - need to determine ranking group and draft value
        const rankingGroup = this.determineRankingGroup(apiRikishi.currentRank);
        const draftValue = this.calculateDraftValue(apiRikishi.currentRank);
        ...
        const { data: newRikishi, error } = await supabase
          .from('rikishi')
  ...
  // Determine ranking group based on current rank
  determineRankingGroup(currentRank) {
    ...
    if (rank.includes('maegashira') && rank.includes('makuuchi')) {
    ...
    if (rank.includes('juryo') || rank.includes('makushita')) return 'Green';
    
    return 'White'; // Sandanme and below
  }

  // Calculate draft value based on rank
  calculateDraftValue(currentRank) {
    ...
    if (rank.includes('maegashira')) {
      ...
      if (match) {
        const number = parseInt(match[1]);
        return Math.max(6 - Math.floor(number / 3), 2); // 6 for M1-3, 5 for M4-6, etc.
      }
      return 4;
    }
    if (rank.includes('juryo')) return 3;
    if (rank.includes('makushita')) return 2;
  ...
  // Full sync operation - sync all rikishi data
  async performFullSync() {
    try {
      console.log('Starting full rikishi sync...');
      
      const apiRikishi = await this.fetchAllRikishi();
      ...
      return { created, updated, errors, bashoId: bashoData.id };
    } catch (error) {
